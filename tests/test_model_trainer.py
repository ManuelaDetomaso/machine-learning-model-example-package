...
def spark():
    os.environ["PYSPARK_TEMP_DIR"] = "C:\\Users\\m679699\\temp_spark"
    os.makedirs(os.environ["PYSPARK_TEMP_DIR"], exist_ok=True)
...
def mock_config():
    return {
        ...
        "ModelGeneral": {"experiment_name": "test-experiment"},
    }
...
def test_split_train_test_data_numerical(spark, mock_config):
    ...
    with patch("diabete_prediction.train_model.load_config", return_value=mock_config):
        with patch("pyspark.sql.DataFrame.toPandas", return_value=pdf):
            trainer = ModelTrainer()
            X_train, X_test, y_train, y_test = trainer.split_train_test_data(
                spark_df, target_type="numerical"
            )
...
def test_split_train_test_data_invalid_type_raises(spark, mock_config):
    pdf = pd.DataFrame({"AGE": [20], "BMI": [22], "target": [1]})
    spark_df = spark.createDataFrame(pdf)
    ...
    with patch("diabete_prediction.train_model.load_config", return_value=mock_config):
        with patch("pyspark.sql.DataFrame.toPandas", return_value=pdf):
            ...
            with pytest.raises(
                ValueError,
                match="Indicate a target type, either numerical or categorical"
            ):
...
def test_mlflow_training_logs_model(mock_mlflow, mock_schema_func):
    mock_schema_func.return_value = Schema(
        [ColSpec("long", "AGE"), ColSpec("double", "BMI")]
    )
    ...
    with patch("diabete_prediction.train_model.load_config", return_value={
        "InputData": {
            "risk_colname": "Risk",
            "target_colname": "target",
            "feature_schema": [
...
def test_save_mlflow_model_registers_model(mock_mlflow, mock_config):
    ...
    with patch("diabete_prediction.train_model.load_config", return_value=mock_config):
        ...
        mock_mlflow.register_model.assert_called_with(
            "runs:/run_456/model", "test-experiment-model"
        )
