...
def mock_config():
    return {
        ...
        "OutputData": {"prepared_inference_data_table_name": "mock_table"},
    }
...
def test_prepare_training_data_incorrect_risk_proportions_warns(spark, mock_config):
    ...
    with patch(
        "diabete_prediction.condig_loader.load_config", return_value=mock_config
    ):
        preparator = DataPreparator()
        ...
        with patch("warnings.warn") as warn_mock:
            ...
            warn_mock.assert_called_once_with(
                "Target lables proportions deviates from expectations."
            )
...
def test_cast_columns_data_types(spark, mock_config):
    ...
    with patch(
        "diabete_prediction.config_loader.load_config", return_value=mock_config
    ):
        preparator = DataPreparator()

        # Mock the saveAsTable chain
        writer_mock = MagicMock()
        sdf.write.format.return_value.mode.return_value.option.return_value.saveAsTable = (
            writer_mock
        )
...
def test_cast_columns_data_types_invalid_type_raises(spark, mock_config):
    mock_config["InputData"]["feature_schema"] = [
    ...
    with patch(
        ...
        preparator = DataPreparator()
        with pytest.raises(
            ValueError, match="Unsupported type 'boolean' for column 'AGE'"
        ):
